        ...
        <main className="flex-grow overflow-y-auto pb-16">
          ...
          <div className="p-4">
            <div className="flex justify-between items-start mb-3">
              <Avatar className={`h-20 w-20 border-4 border-twitter-background ${userProfile.headerImageUrl ? '-mt-12' : 'mt-2'}`}>
                <AvatarImage src={userProfile.avatarUrl} alt={userProfile.name} data-ai-hint={userProfile.avatar_data_ai_hint}/>
                <AvatarFallback>{userProfile.name.substring(0,1)}</AvatarFallback>
              </Avatar>
              <Button variant="outline" className="rounded-full px-4 py-1.5 text-sm font-bold border-twitter-primary text-twitter-primary hover:bg-twitter-primary/10">
                Edit profile
              </Button>
            </div>

            <div>
              <h2 className="text-xl font-bold text-twitter-text-primary">{userProfile.name}</h2>
              <p className="text-sm text-twitter-text-secondary">@{userProfile.handle}</p>
            </div>

            <p className="text-twitter-text-primary my-3 text-sm">{userProfile.bio}</p>

            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-twitter-text-secondary mb-3">
              {userProfile.website && (
                <div className="flex items-center space-x-1">
                  <Link2 size={14} />
                  <a href={`https://${userProfile.website}`} target="_blank" rel="noopener noreferrer" className="text-twitter-primary hover:underline">
                    {userProfile.website}
                  </a>
                </div>
              )}
            ...
            <div className="flex space-x-4 text-sm mb-4">
              <p><span className="font-bold text-twitter-text-primary">{userProfile.followingCount}</span> <span className="text-twitter-text-secondary">Following</span></p>
              <p><span className="font-bold text-twitter-text-primary">{userProfile.followersCount}</span> <span className="text-twitter-text-secondary">Followers</span></p>
            </div>
          </div>
          ...
          <div className="border-b border-twitter-divider flex">
            {tabs.map(tab => (
              ...
              >
                {tab}
                {activeTab === tab && <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-twitter-primary mx-auto w-1/2"></div>}
              </button>
            ))}
          </div>
