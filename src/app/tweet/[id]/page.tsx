...
import { mainTweetDetailData, repliesForTweetData } from '@/contents/tweetDetailsData'; // Import data
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow overflow-y-auto pb-32 bg-twitter-background"> 
        ...
        {currentReplies.map((reply, index) => (
          <ReplyCard key={reply.id} reply={reply} showConnector={index < currentReplies.length || currentReplies.length > 0} />
        ))}
        {currentReplies.length > 0 && (
             <div className="px-4 pb-4 border-b border-twitter-divider">
                <Button variant="link" className="text-twitter-primary text-sm p-0 h-auto hover:underline">
                    1 more reply
                </Button>
            </div>
        )}
