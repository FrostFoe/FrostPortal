  ...
  const messages = [
    {
      ...
      name: "Dev Helper",
      handle: "devhelp",
      avatar: "https://placehold.co/48x48.png?text=DH",
    ...
    },
    {
      ...
      name: "Support Bot",
      handle: "supportbot",
      avatar: "https://placehold.co/48x48.png?text=SB",
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-grow overflow-y-auto pb-16">
          {messages.map((msg) => (
            <div
              key={msg.id}
              className={`flex items-start space-x-3 p-4 border-b border-twitter-divider ${msg.unread ? "bg-twitter-primary/5" : ""}`}
            >
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <span className="text-twitter-text-secondary text-sm ml-1">
                      @{msg.handle}
                    </span>
                  </div>
                  <span className="text-twitter-text-secondary text-xs">
                    {msg.time}
                  </span>
                </div>
                <p
                  className={`text-sm ${msg.unread ? "text-twitter-text-primary font-medium" : "text-twitter-text-secondary"}`}
                >
                  {msg.lastMessage}
                </p>
              </div>
              {msg.unread && (
                <div className="w-2 h-2 bg-twitter-primary rounded-full self-center ml-2"></div>
              )}
            </div>
          ))}
          {messages.length === 0 && (
