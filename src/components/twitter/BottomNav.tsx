...
export function BottomNav() {
  ...
  const navItems = [
    ...
    { href: '/notifications', icon: Bell, label: 'Notifications', badgeCount: 3 },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-14 bg-twitter-background border-t border-twitter-divider flex items-center justify-around font-system max-w-md mx-auto z-50 md:hidden">
      {navItems.map((item) => {
        ...
        return (
          <Link href={item.href} key={item.label} className="flex-1 flex flex-col justify-center items-center h-full relative group">
            <div className="relative">
              <IconComponent 
                size={24} 
                className={isActive ? 'text-twitter-primary' : 'text-twitter-text-secondary group-hover:text-twitter-primary/80'}
                fill={isActive && (item.icon === Home || item.icon === Bell) ? 'currentColor' : 'none'}
              />
