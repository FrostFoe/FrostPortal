    ...
    <div className="p-4 border-b border-twitter-divider font-system bg-twitter-background">
      ...
      <div className="flex items-start space-x-3">
        <Avatar className="h-12 w-12 flex-shrink-0">
          <AvatarImage src={tweet.user.avatarUrl} alt={tweet.user.name} data-ai-hint={tweet.user.avatar_data_ai_hint || "user avatar"}/>
          <AvatarFallback>{tweet.user.name.substring(0, 1)}</AvatarFallback>
        </Avatar>
        <div className="flex-1">
          <div className="flex flex-col">
            <span className="font-bold text-twitter-text-primary text-16px">{tweet.user.name}</span>
            <span className="text-twitter-text-secondary text-14px">@{tweet.user.handle}</span>
          </div>
        </div>
      ...
      <p className="text-twitter-text-primary text-xl mt-3 whitespace-pre-wrap">{tweet.content}</p>
      
      {tweet.imageAttachmentUrl && (
        <div className="mt-3 rounded-xl overflow-hidden border border-twitter-divider">
          <Image src={tweet.imageAttachmentUrl} alt="Tweet image" width={500} height={300} className="object-cover w-full" data-ai-hint={tweet.image_data_ai_hint || "content image"}/>
        </div>
      )}
      ...
      <div className="mt-3 text-sm text-twitter-text-secondary">
        ...
        <span className="text-twitter-primary hover:underline cursor-pointer">{tweet.source}</span>
      </div>
      ...
      {(tweet.retweetCount > 0 || tweet.likeCount > 0) && (
        <div className="mt-3 py-3 border-t border-b border-twitter-divider flex space-x-4">
          {tweet.retweetCount > 0 && (
            <p className="text-sm">
              <span className="font-bold text-twitter-text-primary">{tweet.retweetCount}</span>
              <span className="text-twitter-text-secondary ml-1">Retweets</span>
            </p>
          )}
          {tweet.likeCount > 0 && (
            <p className="text-sm">
              <span className="font-bold text-twitter-text-primary">{tweet.likeCount}</span>
      ...
      <div className="flex justify-around items-center mt-1 text-twitter-text-secondary py-1">
        <Button variant="ghost" size="icon" className="text-twitter-text-secondary hover:text-twitter-primary hover:bg-twitter-primary/10 rounded-full w-10 h-10">
          <MessageCircle size={22} />
        </Button>
        <Button variant="ghost" size="icon" className="text-twitter-text-secondary hover:text-twitter-retweet hover:bg-twitter-retweet/10 rounded-full w-10 h-10">
          <Repeat2 size={22} />
        </Button>
        <Button variant="ghost" size="icon" className="text-twitter-text-secondary hover:text-twitter-like hover:bg-twitter-like/10 rounded-full w-10 h-10">
          <Heart size={22} />
        </Button>
        <Button variant="ghost" size="icon" className="text-twitter-text-secondary hover:text-twitter-primary hover:bg-twitter-primary/10 rounded-full w-10 h-10">
          <Share size={22} />
