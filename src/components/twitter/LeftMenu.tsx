...
import { User, List, Hash, Bookmark, Zap, Settings, HelpCircle, Sun, QrCode, Moon, Feather } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-twitter-background text-twitter-text-primary font-system">
      ...
      <div className="p-3 hidden md:block">
        <Link href="/" passHref>
          <Feather size={30} className="text-twitter-primary" />
        </Link>
      </div>

      <div className="p-4 space-y-2 border-b border-twitter-divider md:border-b-0">
        <Avatar className="h-12 w-12">
          <AvatarImage src={currentUser.avatarUrl} alt={currentUser.name} data-ai-hint={currentUser.avatar_data_ai_hint} />
          <AvatarFallback>{currentUser.name.substring(0,1)}</AvatarFallback>
        </Avatar>
        <div>
          <p className="font-bold text-18px">{currentUser.name}</p>
          <p className="text-twitter-text-secondary text-14px">@{currentUser.handle}</p>
        </div>
        <div className="flex space-x-4 text-twitter-text-secondary text-14px">
          <p><span className="text-twitter-text-primary font-semibold">{currentUser.followingCount}</span> Following</p>
          <p><span className="text-twitter-text-primary font-semibold">{currentUser.followersCount}</span> Followers</p>
        </div>
      </div>
        ...
        {menuItems.map((item) => {
          const linkContent = (
            <Link href={item.href} passHref>
              <Button variant="ghost" className="w-full justify-start h-[55px] text-lg px-4 text-twitter-text-primary hover:bg-twitter-primary/10 rounded-full">
          ...
          <div className="px-3 mt-4 hidden md:block">
            <Link href="/compose/tweet" passHref>
              <Button className="bg-twitter-primary text-white rounded-full h-12 w-full font-bold text-lg hover:bg-twitter-primary/90">
                <Feather size={20} className="mr-2 md:hidden"/>
                <span>Tweet</span>
              </Button>
            </Link>
          </div>
      ...
      {inSheetContext && (
        <>
          ...
          {bottomMenuItems.map((item) => (
             <SheetClose asChild key={item.label}>
              <Link href={item.href}>
                <Button variant="ghost" className="w-full justify-start h-[55px] text-16px px-4 text-twitter-text-primary hover:bg-twitter-primary/10">
      ...
      {/* Desktop Fixed Sidebar Footer */}
      {!inSheetContext && (
        <div className="mt-auto p-3 border-t border-twitter-divider">
           <Link href="/settings" passHref>
              <Button variant="ghost" className="w-full justify-start h-[55px] text-16px px-4 text-twitter-text-primary hover:bg-twitter-primary/10">
                  <Settings size={24} className="mr-4" /> Settings and privacy
              </Button>
           </Link>
           <Link href="/help" passHref>
              <Button variant="ghost" className="w-full justify-start h-[55px] text-16px px-4 text-twitter-text-primary hover:bg-twitter-primary/10">
                  <HelpCircle size={24} className="mr-4" /> Help Center
              </Button>
           </Link>
        </div>
      )}
