    ...
    return (
      <>
        {parts.map((part, index) => {
          ...
          if (part.startsWith('##') && part.endsWith('##')) { // For hashtags
            return <span key={index} className="text-twitter-primary">{part.substring(2, part.length - 2)}</span>;
          }
           if (part.startsWith('@@') && part.endsWith('@@')) { // For links
            return <span key={index} className="text-twitter-primary underline">{part.substring(2, part.length - 2)}</span>;
          }
          return part;
        })}
      </>
...
const NotificationItemCardComponent: React.FC<NotificationItemCardProps> = ({ notification }) => {
  return (
    <div className="flex space-x-3 p-4 border-b border-twitter-divider bg-twitter-background hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors duration-150">
      <div className="w-8 flex justify-end pt-1">
        {notification.iconType === 'like' && <Heart size={20} className="text-twitter-like fill-current" />}
        {/* Add other icons for follow, mention, reply here */}
      </div>
      <div className="flex-1">
        <Avatar className="h-8 w-8 mb-1">
          <AvatarImage src={notification.avatarUrl} alt={notification.userName} data-ai-hint={notification.data_ai_hint || "user avatar"}/>
          <AvatarFallback>{notification.userName.substring(0, 1).toUpperCase()}</AvatarFallback>
        </Avatar>
        
        <div className="flex items-center space-x-1 mb-0.5">
          <span className="font-bold text-twitter-text-primary text-sm">{notification.userName}</span>
          {notification.isVerified && <BadgeCheck size={16} className="text-twitter-primary fill-current" />}
          {notification.source && <span className="text-twitter-text-secondary text-sm">{notification.source}</span>}
          <span className="text-twitter-icon-gray text-xs">&middot; {notification.timestamp}</span>
        </div>
