...
export function TopBar({ title, rightActionIcon: RightActionIcon, onRightActionClick }: TopBarProps) {
  return (
    <div className="h-14 bg-twitter-background/80 backdrop-blur-md text-twitter-text-primary px-4 flex items-center justify-between sticky top-0 z-40 border-b border-twitter-divider">
      ...
      {title === "Home" ? (
        ...
        <h1 className="text-lg font-bold font-system text-twitter-text-primary">{title}</h1>
      )}

      {RightActionIcon ? (
        <Button variant="ghost" size="icon" onClick={onRightActionClick} className="p-0 h-8 w-8">
